...
def update():
    ...
    data_subfolder = ['kinetics', 'data', 's1', 'video', 'OPN', 'videovector', 'videovector'][opt.data_type]
    opt.data = os.path.join(opt.dataset_root, 'feat', data_subfolder)

    opt.gt = os.path.join(opt.data, opt.gt)

    opt.ext = ['gz', 'gz', 'txt', 'avi', 'txt', 'txt', 'txt'][opt.data_type]
    opt.feature_dim = [400, 64, 64, 0, 1024, 4096, 64][opt.data_type]
    opt.subfolder = 'ascii'
